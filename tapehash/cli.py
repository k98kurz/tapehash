...
def help_tapehash1():
    print(
        'Use: command | tapehash1 OR tapehash1 --preimage {str} OR tapehash1 '
        '--file path/to/file'
    )
    print('\t--from:hex - parse preimage as hexadecimal')
    print('\t--code_size {int} - set the code_size parameter (default=20)')
    print('\t-cs {int} - set the code_size parameter (default=20)')
...
def help_tapehash2():
    print(
        'Use: command | tapehash2 OR tapehash2 --preimage {str} OR tapehash2 '
        '--file path/to/file'
    )
    print('\t--from:hex - parse preimage as hexadecimal')
    ...
    print('\t-tsm {int} - set the tape_size_multiplier parameter (default=2)')
    print('\t--to:raw - write the raw hash digest bytes to stdout')
    print('\t--difficulty - output the difficulty of the hash digest')
    print('\t--check {int} - outputs 1 if the hash digest meets the '
        'difficulty threshold; outputs 0 and exits with error code 2 '
...
def help_tapehash3():
    print(
        'Use: command | tapehash3 OR tapehash3 --preimage {str} OR tapehash3 '
        '--file path/to/file'
    )
    print('\t--from:hex - parse preimage as hexadecimal')
    print('\t--code_size {int} - set the code_size parameter (default=64)')
    print('\t-cs {int} - set the code_size parameter (default=64)')
    print('\t--tape_size_multiplier {int} - set the tape_size_multiplier' +
        ' parameter (default=2)'
    )
    print('\t-tsm {int} - set the tape_size_multiplier parameter (default=2)')
...
def run_tapehash3() -> None:
    ...
    if len(argv) > 1:
        ...
        if '--to:raw' in argv:
            stdout.buffer.write(tapehash3(
                preimage, code_size, tape_size_multiplier
            ))
            return

        if '--difficulty' in argv:
            diff = calculate_difficulty(tapehash3(
                preimage, code_size, tape_size_multiplier
            ))
            print(diff)
            return
        ...
        if '--check' in argv:
            ...
            if len(argv) < idx + 2:
                ...
                exit(1)
            target = int(argv[idx+1])
            digest = tapehash3(preimage, code_size, tape_size_multiplier)
            if check_difficulty(digest, target):
                print(1)
                return
            else:
